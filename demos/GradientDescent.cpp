...
struct Dataset {
    ...
    Dataset() {
        ...
        for (size_t i = 0; i < size; i++) {
            rand_x = std::rand() % 20;
            // Add randomness to the bias term in the hypothesis
            rand_y = f(2 + std::rand() % 25, 5, rand_x);
...
std::vector<double>
optimize(std::vector<double> theta, Dataset dt, unsigned int maxSteps,
         double eps) {
...
int main() {
    ...
    std::cout << "Result: " << "(" << results[0] << ", " << results[1] << ")"
              << std::endl;

    // For plotting data
    std::ofstream out("out_gd.dat");
    for (size_t i = 0; i < dt.size; i++) {
        out << dt.x[i] << "\t" << f(results[0], results[0], dt.x[i])
            << std::endl;
    }
    out.close();
