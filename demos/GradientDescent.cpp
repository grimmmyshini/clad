...
struct Dataset {
  ...
  Dataset() {
    ...
    for (size_t i = 0; i < size; i++) {
      ...
      rand_y = f(5 + std::rand() % 25, 10, rand_x);
...
// theta_x are the feature parameters, t is the generated dataset and
...
// theta_x are the features to learn; x, y are the inputs and outputs of f
...
// theta is the feature parameter list and maxSteps is the maximum steps to
// perform
std::vector<double> optimize(std::vector<double> theta, Dataset dt,
  ...
  do {
    ...
    hasConverged =
        abs(diff[0] - theta[0]) <= eps && abs(diff[1] - theta[1]) <= eps;
...
int main() {
  ...
  Dataset dt;
  auto results = optimize(theta, dt, 2000, 1e-6);
