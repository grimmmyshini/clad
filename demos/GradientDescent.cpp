...
// gnuplot -e "plot 'dataset_gd.dat' with points pt 7; replot 'out_gd.dat' using 1:2 with lines; pause -1"
...
// theta_0, theta_1 are the features to learn and x is the input data
...
// size represents the number of samples
...
struct Dataset {
    ...
    Dataset() {
        ...
        for(size_t i = 0; i < size; i++) {
            ...
            x.push_back( (std::rand()%20) );
            y.push_back( f(2 + std::rand()%25, 5, x[i]) );
...
// theta_x are the feature parameters
// dt is the generated dataset
...
void performStep(double& theta_0, double& theta_1, Dataset dt, T clad_grad) {
    ...
    for (size_t i = 0; i < dt.size; i++){
        ...
        clad_grad.execute(theta_0, theta_1, dt.x[i], dt.y[i], J_theta);
...
// theta_0, theta_1 are the features to learn
// x, y are the inputs and outputs of f respectively
...
// theta contains the feature parameters
// maxSteps are the maximum steps to perform
std::vector<double> optimize(std::vector<double> theta, Dataset dt, unsigned int maxSteps, double eps) {
    ...
    do {
        ...
        std::cout << "Steps #" << currentStep << " Theta 0: " << theta[0] << " Theta 1: " << theta[1] << std::endl;

        hasConverged = abs(diff[0] - theta[0]) <= eps && abs(diff[1] - theta[1]) <= eps;
...
int main() {
  ...
  for(size_t i = 0; i < dt.size; i++){
      ...
      out << dt.x[i] << "\t" << f(results[0], results[0], dt.x[i]) << std::endl;
  }
  out.close();
